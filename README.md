...
The imprint works by hijacking the 2nd-order Lagrangian perturbation theory (2LPT) corrections. The primary (1LPT) initial conditions remain completely pure and physical. Because the 2LPT correction scales quadratically with the growth factor ($D_1(z)^2$), the text offset is practically zero at $z = 10$ and mathematically forces the particles to converge onto the letter strokes exactly as $D_1(z) \rightarrow 1$. The surrounding cosmic web is unaffected. The feature is **opt-in and off by default**; when unchecked the simulation is identical to the standard run.
